...
def main():
    ...
    if teacher_path is not None:
        ...
        teacher_model = SDARForCausalLM.from_pretrained(
            teacher_path, trust_remote_code=True, torch_dtype="auto"
        )
        teacher_model.eval()
    ...
    # ExOPD-related hyperparameters (optional, keep defaults if not provided)
    alpha_exopd = config.training.get("alpha_exopd", 1.0)
    lambda_teacher = config.training.get("lambda_teacher", 0.0)
        ...
        if (teacher_model is not None) and (lambda_teacher > 0.0):
            with torch.no_grad():
                ...
                teacher_logits = torch.cat(
                    [teacher_logits[:, :L0, :], teacher_logits[:, L0 + L1 :, :]],
                    dim=1,
                )
